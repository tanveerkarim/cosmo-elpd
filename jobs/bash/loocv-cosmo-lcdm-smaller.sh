...
#SBATCH --job-name=smaller-lcdm-pc
#SBATCH --nodes=1
...
module purge
module load StdEnv/2023 python/3.13 scipy-stack mpi4py
...
modelName="lcdm-smaller"
echo "Model is $modelName"

# 4. Critical environment overrides
export COBAYA_PACKAGES_PATH=/scratch/tanveerk/cobaya-$modelName-packages
...
find /scratch/tanveerk/DESI-DR2_SN-Pantheon+_CMB-CamSpecLensing/$modelName/ -name "*.locked" -type f -delete
find /scratch/tanveerk/cobaya-$modelName-packages/data/planck_2018_CamSpec2021/ -name "*.locked" -type f -delete
...
srun python -m cobaya run /scratch/tanveerk/bayesian-model-workspace/DESI-DR2_Pantheon+_CMB-CamSpec-$modelName.yaml --resume
RUN_EXIT=$?
